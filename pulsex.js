...
const PULSEX_ROUTER_ABI = fs
  .readFileSync("PulseXRouter_v2_abi.json")
  .toString();
const PULSEX_ROUTER_CONTRACT = new ethers.Contract(
...
async function swapTokens(token1, token2, amount, type, slippage = "50") {
  try {
    ...
    if (type === "getDai") {
      rawTxn =
        await PULSEX_ROUTER_CONTRACT.populateTransaction.swapExactETHForTokens(
          amountOutMinHex,
          path,
          to,
          deadline,
      ...
      rawTxn =
        await PULSEX_ROUTER_CONTRACT.populateTransaction.swapExactTokensForETHSupportingFeeOnTransferTokens(
          amountIn,
          amountOutMinHex,
          path,
          to,
    ...
    if (receipt) {
      ...
      console.log(
        `Navigate to https://otter.pulsechain.com/tx/${sendTxn.hash} to see your transaction`
      );
      return receipt
...
async function getDaiPlsBalances() {
  let [daiBalance, plsBalance] = await Promise.all([
    DAI_CONTRACT.balanceOf(walletAddress),
    provider.getBalance(walletAddress),
  ]);
  daiBalance = ethers.utils.formatUnits(daiBalance, 18);
  plsBalance = ethers.utils.formatUnits(plsBalance, 18);
  return { daiBalance, plsBalance };
}
...
async function getDai() {
    ...
    return swapTokens(DAI, WPLS, plsBalanceToUse.toString(), "getDai");
...
async function getPls() {
    ...
    return swapTokens(WPLS, DAI, daiBalanceFormatted, "getPls");
